...
# Модуль для создания титульной страницы оригинального типа с рамками
# 
# Демонстрирует использование абсолютных координат (в сантиметрах) с добавлением рамок.
# Все позиции задаются в сантиметрах от краев страницы.
# Рамка рисуется с отступами: top=5мм, right=5мм, bottom=5мм, left=20мм
# Толщина линии рамки: 0.1мм
#
# Используется для: атрибута :custom_title_page: original_frame
module ConvertTitlePageOriginalFrame
  ...
  def ink_title_page(doc)
    ...
    # # 2. Блок названия документа - позиционируем в центре страницы
    ...
    # # 5. Блок количества листов - позиционируем в центре страницы
